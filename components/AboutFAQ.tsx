...
export default function AboutFAQ({ faqs }: { faqs: AboutFaqItem[] }) {
    const [openIndex, setOpenIndex] = React.useState<number | null>(0);

    return (
        <section id="about-faq" className="py-24 px-4 bg-slate-900 border-y border-slate-800">
            <div className="max-w-3xl mx-auto">
                <div className="text-center mb-14">
                    ...
                    <p className="text-slate-400 text-lg">
                        Straight answers about our certification, guarantees and quoting process.
                    </p>
                </div>
                <div className="space-y-4">
                    {faqs.map((faq, index) => {
                        ...
                        return (
                            <div key={index} className="bg-slate-950/70 border border-slate-800 rounded-2xl overflow-hidden">
                                <button
                                    onClick={() => setOpenIndex(isOpen ? null : index)}
                                    className="w-full flex items-center justify-between gap-4 text-left p-6 hover:bg-slate-900/50 transition-colors"
